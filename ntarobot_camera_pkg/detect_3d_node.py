...
class Detect3DNode(LifecycleNode):
    ...
    def process_detections(
        ...
        transform = self.get_transform('camera_depth_optical_frame')
        if transform is None:
            return []
        
        new_detections = []

        # Check new type of reading of img
        np_arr = np.frombuffer(depth_msg.data, np.uint8)
        depth_image = cv2.imdecode(np_arr, cv2.IMREAD_UNCHANGED)
        if depth_image is None:
                self.get_logger().error("Failed to decode PNG image.")
                return
        # self.get_logger().info(f"Dimensiones of img: {depth_image.shape}")
