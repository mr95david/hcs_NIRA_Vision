...
class DebugNode(LifecycleNode):
    ...
    def on_configure(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self._compressed_dbg_pub = self.create_publisher(CompressedImage, "dbg_image/compressed", 10)
    ...
    def on_activate(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self.image_sub = message_filters.Subscriber(
            #self, Image, "/ntarobot/img", qos_profile=self.image_qos_profile)
            self, CompressedImage, "/color/image_raw/compressed", qos_profile=self.image_qos_profile)
        self.detections_sub = message_filters.Subscriber(
            self, DetectionArray, "/detections_3d", qos_profile=self.image_qos_profile)
    ...
    def on_cleanup(self, state: LifecycleState) -> TransitionCallbackReturn:
        ...
        self.destroy_publisher(self._compressed_dbg_pub )
    ...
    def detections_cb(self, img_msg: CompressedImage, detection_msg: DetectionArray) -> None:
        ...
        compressed_image_msg.header.frame_id = "camera_color_optical_frame"
        compressed_image_msg.format = "rgb8; jpeg compressed bgr8"

        compressed_image = cv2.imencode('.jpg', cv_image)[1].tobytes()
        compressed_image_msg.data = np.frombuffer(compressed_image, dtype=np.uint8).tolist()


        self._dbg_pub.publish(self.cv_bridge.cv2_to_imgmsg(cv_image, encoding="bgr8"))
        self._compressed_dbg_pub.publish(compressed_image_msg)
